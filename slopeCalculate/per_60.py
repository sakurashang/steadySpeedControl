...
for i in range(0, len(data), 60):
    if i + 60 <= len(data):
        ...
        # 取第三十条数据的速度指令
        command_speed = subset['cmd_v'].iloc[29]
        # 计算六十条数据中的实际速度平均值
...
# 导出到CSV文件
result_data.to_csv('../imuRecord/slopeData/行驶时记录数据2023-10-14 16:50:09.csv', index=False)
