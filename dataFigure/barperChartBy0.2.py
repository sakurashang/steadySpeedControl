...
# 每0.2一段，对数据分组
grouped_data1 = data1.groupby((data1['v'] // 0.2) * 0.2).size() / total_data1 * 100
grouped_data2 = data2.groupby((data2['v'] // 0.2) * 0.2).size() / total_data2 * 100

# 绘制第一个柱状图
plt.figure(figsize=(12, 6))
plt.subplot(1, 2, 1)
plt.bar(grouped_data1.index, grouped_data1.values, width=0.15)
plt.xlabel('Speed')
...
# 绘制第二个柱状图
plt.subplot(1, 2, 2)
plt.bar(grouped_data2.index, grouped_data2.values, width=0.15)
plt.xlabel('Speed')
plt.ylabel('Percentage of Total Data')
