...
# 指定包含CSV文件的文件夹路径
folder_path = '../../imuRecord/10-16/'  # 请替换为实际的文件夹路径
...
for file in csv_files:
    ...
    averaged_speed = []
    averaged_timestamp = []
    for i in range(0, len(df['v']) - window_size, window_size):
        window_data = df['v'][i:i + window_size]
        average_speed = window_data.mean()
        averaged_speed.append(average_speed)
        # 每五条数据选择第三条的时间戳作为新的时间戳
        averaged_timestamp.append(df['GPSTime'].iloc[i + 2])


    # 创建新的DataFrame保存平均值和新的时间戳数据
    averaged_df = pd.DataFrame({'GPSTime': averaged_timestamp,'v': averaged_speed})
