...
if __name__ == "__main__":
    ...
    if dataset is not None:
        # 初始化前四个点的信息
        prev_utm_x = df.at[0, 'x']
        prev_utm_y = df.at[0, 'y']
        prev_elevation = get_elevation_at_coordinates(prev_utm_x, prev_utm_y, dataset)

        prev2_utm_x = df.at[1, 'x']
        prev2_utm_y = df.at[1, 'y']
        prev2_elevation = get_elevation_at_coordinates(prev2_utm_x, prev2_utm_y, dataset)

        prev3_utm_x = df.at[2, 'x']
        prev3_utm_y = df.at[2, 'y']
        prev3_elevation = get_elevation_at_coordinates(prev3_utm_x, prev3_utm_y, dataset)
        ...
        while i <= length-2:  # 当跟踪的最后一个点时候，结束跟踪，小车制动。因为每步只跟踪一个点，所以i的范围可以从0~length-1,设为len-2保险
            ...
            # 获取当前点的高程
            elevation = get_elevation_at_coordinates(utm_x, utm_y, dataset)

            if elevation is not None:
                # 计算两点之间的水平距离
                distance = ((utm_x - prev_utm_x) ** 2 + (utm_y - prev_utm_y) ** 2) ** 0.5

                # 计算坡度
                slope = calculate_slope(prev3_elevation, elevation, distance)

                print(
                    f"At UTM coordinates ({utm_x}, {utm_y}), elevation is: {elevation} meters, slope is: {slope} degrees")

                # 更新前四个点的信息
                prev_utm_x = prev2_utm_x
                prev_utm_y = prev2_utm_y
                prev_elevation = prev2_elevation

                prev2_utm_x = prev3_utm_x
                prev2_utm_y = prev3_utm_y
                prev2_elevation = prev3_elevation

                prev3_utm_x = utm_x
                prev3_utm_y = utm_y
                prev3_elevation = elevation
            ...
            # 示例用法
            slope_value = slope  # 假设的坡度值，具体值需要根据实际情况获取
